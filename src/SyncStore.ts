	...
	set(vpath: string, meta: Meta) {
		...
		this.ydoc.transact(() => {
			...
			if (pendingGuid && pendingGuid === meta.id) {
				...
				// The ticket is now stale; if nothing clears it, the path
				// auto-republishes when this committed entry is deleted.
				this.warn("committed claim shadows a pending-upload ticket", vpath, {
					pending: pendingGuid,
					committed: meta.id,
				});
			}
